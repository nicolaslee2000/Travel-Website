...
import './backResultItem.css';
import { CardActions, CardHeader } from '@mui/material';
import { useSelector } from 'react-redux';

const BackResultItem = () => {
  const flightRedux = useSelector((state) => {
    return state.searchReducer3;
  });

  const departureTime = new Date(
    flightRedux.flightPrice.flightOffers[0].itineraries[1].segments[0].departure.at
  );
  const arrivalTime = new Date(
    flightRedux.flightPrice.flightOffers[0].itineraries[1].segments[0].arrival.at
  );
  const leadTime =
          ...
          >
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Typography variant='h4' component='div'>
                {
                  flightRedux.flightPrice.flightOffers[0].itineraries[1]
